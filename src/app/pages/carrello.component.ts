  ...
  template: `
    <ng-container *ngIf="!orderSend">
      <div class="card text-center">
        <div class="card-header">Cart <i class="bi bi-apple"></i> Products</div>
        ...
        <div class="w-50 mx-auto mt-5">
          <ul class="list-group">
            <li *ngFor="let item of cart" class="list-group-item d-flex justify-content-between">
              ...
              <span>
                {{ item.price | currency: "EUR" }} <button (click)="removeProduct(item)" class="btn btn-danger"><i class="bi bi-x-circle-fill "></i></button
              ></span>
            </li>
...
export class CarrelloComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    for (let i = 0; i < this.cart.length; i++) {
      //cicle for add money on sumPrice
